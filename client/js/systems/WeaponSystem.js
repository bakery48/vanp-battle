// WeaponSystem.js - Weapon and bullet management
class WeaponSystem {
  ...
  _fire(px, py, target) {
    ...
    const spread = count > 1 ? (Math.PI / 8) : 0; // 22.5 deg spread between bullets
    ...
    for (let i = this.bullets.length - 1; i >= 0; i--) {
      ...
      for (const e of enemies) {
        ...
        if (Math.sqrt(dx * dx + dy * dy) < e.size + 5) {
          b.hitEnemies.add(e);
          const killed = this.scene.enemySpawner ? this.scene.enemySpawner.hitEnemy(e, b.damage) : false;

          if (killed && this.scene.onEnemyKilled) {
  ...
  // For battle scene: fire toward a specific target position
